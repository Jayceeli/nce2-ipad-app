(() => {
  const LANG_KEY = 'learning_lang_mode';
  function getLang() {
    const v = localStorage.getItem(LANG_KEY) || localStorage.getItem('nce_lang_mode');
    return (v === 'en' || v === 'cn' || v === 'bi') ? v : 'bi';
  }
  function setLang(v) {
  ...
  function initSegmented(container) {
    const segs = container?.querySelectorAll('[data-mode]');
    if (!segs) return;
    const current = getLang(); applyLang(current);
    segs.forEach((btn) => {
      btn.classList.toggle('active', btn.dataset.mode === current);
      btn.addEventListener('click', () => {
        setLang(btn.dataset.mode || 'bi');
